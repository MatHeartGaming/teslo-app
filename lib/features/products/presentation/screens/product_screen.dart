        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
                onPressed: () async {
                  final photoPath =
                      await CameraGalleryServiceImplementation().selectPhoto();
            ...
            IconButton(
                onPressed: () async {
                  final photoPath =
                      await CameraGalleryServiceImplementation().takePhoto();
                  if (photoPath == null) return;
                  ref
                      .read(productFormProvider(productState.product!).notifier)
                      .updateProductImage(photoPath);
        ...
        children: [
          ...
          CustomProductField(
            ...
            onChanged: (price) => ref
                ...
                .onPriceChanged(double.tryParse(price) ?? -1),
          ),
          const SizedBox(height: 15),
          const Text('Extras'),
          _SizeSelector(
          ...
          CustomProductField(
            ...
            onChanged: (stock) => ref
                .read(productFormProvider(product).notifier)
                .onStockChanged(int.tryParse(stock) ?? -1),
          ),
          CustomProductField(
            maxLines: 6,
            label: 'Descripción',
...
class _ImageGallery extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final emptyImage = ClipRRect(
        borderRadius: const BorderRadius.all(Radius.circular(20)),
        child: Image.asset('assets/images/no-image.jpg', fit: BoxFit.cover));
    if (images.isEmpty) return emptyImage;
    return PageView(
